...
//        validate(enrollmentDto); // Check For OverLapping Shifts with existing shifts if any.
        List<Enrollment> existingEnrollment = enrollmentDao.findAllByDoctorId(enrollmentDto.getDoctorId());

        Set<Integer> clinicIds = existingEnrollment.stream().map(Enrollment::getClinicId).collect(Collectors.toSet());

        if (!clinicIds.contains(enrollmentDto.getClinicId())) {
            ...
            log.info("Doctor already Enrolled with this clinic!");
            return null;
        }
    }
